...
optparser.add_option(
    '-q', '--fin', help='Read query from file (defaults to stdin)')
optparser.add_option(
    '-o', '--fout', help='output file name (defaluts to stdout)')
optparser.add_option('-u', '--url', help='url of the sparql endpoint')
optparser.add_option('-t', '--format', type='choice', choices=[
                     'xml', 'json', 'brtr'],
                     help="format to return one of: ['xml','json','brtr']")
optparser.set_defaults(format='xml')


# time echo "select * { ?s a ?o } limit 2" | sparql -t xml -u $SPARQL1 - |
# xsltproc xml-to-html.xsl -
